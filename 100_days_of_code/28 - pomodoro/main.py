...
def start_timer():
    ...
    if reps % 8 == 0:
        label_timer.config(text="Break", font=(FONT_NAME, 40, "bold"), bg=YELLOW, fg=PINK)
        count_down(short_break_sec)
    elif reps % 2 == 0:
        label_timer.config(text="Break", font=(FONT_NAME, 40, "bold"), bg=YELLOW, fg=RED)
        count_down(long_break_sec)
    else:
        label_timer.config(text="Work", font=(FONT_NAME, 40, "bold"), bg=YELLOW, fg=GREEN)
        count_down(work_sec)
...
label_checkmark = Label(font=(FONT_NAME, 16), fg=GREEN, bg=YELLOW)
